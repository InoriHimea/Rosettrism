...
async function mockApi(page) {
  ...
  await page.route('**/api/cache/101', (route) => route.fulfill({
    contentType: 'application/json',
    body: JSON.stringify({ upstream: { ...cacheEntry, body_text_preview: '[00:00] Visual smoke lyric' } }),
  }));
...
test('Dashboard loads, Settings token input works, and Fetch renders', async ({ page }) => {
  ...
  await expect(page.getByRole('radio', { name: /Cyber dark/ })).toBeChecked();
...
test('visual smoke captures dashboard, settings, karaoke stage, and cache detail', async ({ page }) => {
  ...
  await expect(page.getByText('Particles drift behind the stage')).toBeVisible();
  ...
  await page.getByText('Visual Smoke').click();
  await expect(page.getByRole('heading', { name: '缓存' })).toBeVisible();
  await expect(page.getByText('Content preview').or(page.getByText('内容预览'))).toBeVisible();
  await expect(page.getByText('[00:00] Visual smoke lyric')).toBeVisible();
